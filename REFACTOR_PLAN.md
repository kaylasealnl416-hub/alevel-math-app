...
**已完成:**
- ✅ 创建 `src/App.jsx` - 主应用入口
- ✅ 更新 `src/main.jsx` - 使用新入口
- ✅ 创建目录结构 `src/components/views/`, `src/styles/`

**未完成 (原因):**
- ❌ 完整提取视图组件 - 组件间依赖过于紧密
  - 所有视图共享: CURRICULUM, SUBJECTS, T (翻译), styles
  - 直接拆分会导致循环依赖
  - 需要大量手动重构导入路径
...
```
src/
├── App.jsx                    # 新增: 主入口 (Wrapper)
├── main.jsx                   # 已更新: 导入 App
├── alevel-math-app.jsx        # 保留: 单文件组件
├── components/
│   ├── views/                 # 空目录 (待未来使用)
│   └── ...
└── ...
...
单文件组件的直接拆分复杂度超出预期。建议：
1. **短期**: 保持现状，使用 App.jsx 入口
2. **长期**: 渐进式重构，每次改一个小组件
